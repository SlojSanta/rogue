        ...
        for (var j = 0; j < map[0].length; j++) {
            if (map[i][j] == items[5].id) {
                try { if (map[i][j - 1] === items[4].id) {
                    counter++
                    if (map[i][j - 1] === items[4].id) return [backGetElementIdForGenerateMap(map[0].length, i, j - 1), i, j - 1]
                } } catch {}
                try { if (map[i][j + 1] === items[4].id) {
                    counter++
                    if (map[i][j + 1] === items[4].id) return [backGetElementIdForGenerateMap(map[0].length, i, j + 1), i, j + 1]
                ...
                try { if (map[i - 1][j] === items[4].id) {
                    counter++
                    if (map[i - 1][j] === items[4].id) return [backGetElementIdForGenerateMap(map[0].length, i - 1, j), i - 1, j]
                } } catch {}
                try { if (map[i + 1][j] === items[4].id) {
                    counter++
                    if (map[i + 1][j] === items[4].id) return [backGetElementIdForGenerateMap(map[0].length, i + 1, j), i + 1, j]
...
function frontAddItemForUpdateMap (newId, className, pickHP, pickSW, currentId) {
    ...
    if (className === 'tileP') {
        ...
        if (pickHP) {
            if ((parseStringToInt(divHelth.style.height)) < 10) {
                divHelth.style.height = (parseStringToInt(healthHeight) + 1) + 'px'
            }
...
function createElementAddclasslistAddstyleheigth (parameterCreateItem = 0, parameterAddClassList = 0, parameterAddstyle = 0) {
    var item = document.createElement(parameterCreateItem)
    item.classList.add(parameterAddClassList)
